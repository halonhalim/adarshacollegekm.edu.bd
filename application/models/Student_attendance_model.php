...
class Student_attendance_model extends CI_Model {
	...
	var $column_order = array(null, 'date', 'class_name', 'total_teacher', 'total_presence');
	var $column_search = array('date', 'class_name', 'total_teacher', 'total_presence');
	var $order = array('student_attendance.id' => 'DESC');

	private function search() {
		$this->db->select('class_name, student_attendance.*');
		$this->db->from($this->table);
		$this->db->join('classes', 'student_attendance.class_id=classes.id', 'LEFT');
		$i = 0;
	...
	// Script for site
	public function attendance_by_class ($class_id=NULL) {
		$this->db->select('*');
		$this->db->from($this->table);
