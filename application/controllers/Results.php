...
class Results extends CI_Controller {
	...
	var $dir = 'admin/results/';
	var $file_path = 'results';
	var $breadcrumb = 'ফলাফল';
	...
	public function get_all() {
		$list = $this->results_model->get_all();
		$data = array();
		$i = $_POST['start'];
		foreach ($list as $item) {
			...
			$row = array();
			$row[] = sprintf("%02d", $i);
			$row[] = eng_to_bng($item->year);
			$row[] = $item->class_name;
			$row[] = $item->title;
			$row[] = eng_to_bng(date('d-m-Y', strtotime($item->published)));
			
			if($item->file_name) {
				$row[] = "<a href='".base_url().'assets/filemanager/results/'.$item->file_name."' target='_blank'><img src='".base_url().'assets/filemanager/pdf.png'."' width='25px' class='img-responsive' /></a> " ;
			} else {
				$row[] = '';
			}
			$action = "<form method='post' action='".base_url()."results/edit' class='actionForm' onClick='return altEdit();' >"
			...
			$action .= "<form method='post' action='".base_url()."results/delete' class='actionForm' onClick='return altDelete();' >"
						."<input type ='hidden' name='id' value='".md5($item->id)."' />"
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->results_model->count_all(),
			"recordsFiltered" => $this->results_model->count_filtered(),
			"data" => $data,
		);
	...
	public function save() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->results_model->save($data)) {
	...
	public function edit() {
		...
		$data['edit'] = $this->results_model->get_by_id($id);
		$data['breadcrumb'] = $this->breadcrumb;
	...
	public function update() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->results_model->update($data, $id)) {
				$this->form_validation->resetForm();
	...
	public function delete() {
		$id = $this->input->post('id');
		$results = $this->results_model->get_by_id($id);
		
		if ($this->results_model->delete($id) == FALSE) {
			$this->session->set_tempdata("msg", "<span class='error'>" . exception() . "</span>", 5);
		} else {
			$this->master_model->delete_file($this->file_path, $results->file_name);
