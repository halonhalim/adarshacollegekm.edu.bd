...
class Notice extends CI_Controller {
	...
	var $dir = 'admin/notice/';
	var $file_path = 'notice';
	var $breadcrumb = 'নোটিশ বোর্ড';
	...
	public function get_all() {
		$list = $this->notice_model->get_all();
		$data = array();
		$i = $_POST['start'];
		foreach ($list as $item) {
			...
			if($item->file_name) {
				$row[] = "<a href='".base_url().'assets/filemanager/notice/'.$item->file_name."' target='_blank'><img src='".base_url().'assets/filemanager/pdf.png'."' width='25px' class='img-responsive' /> </a>" ;
			} else {
				$row[] = '';
			}
			$action = "<form method='post' action='".base_url()."notice/edit' class='actionForm' onClick='return altEdit();' >"
			...
			$action .= "<form method='post' action='".base_url()."notice/delete' class='actionForm' onClick='return altDelete();' >"
						."<input type ='hidden' name='id' value='".md5($item->id)."' />"
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->notice_model->count_all(),
			"recordsFiltered" => $this->notice_model->count_filtered(),
			"data" => $data,
		);
	...
	public function delete() {
		$id = $this->input->post('id');
		$notice = $this->notice_model->get_by_id($id);
		
		if ($this->notice_model->delete($id) == FALSE) {
			$this->session->set_tempdata("msg", "<span class='error'>" . exception() . "</span>", 5);
		} else {
			$this->master_model->delete_file($this->file_path, $notice->file_name);
